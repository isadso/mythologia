...
(function registerIrishEntities() {
  ...
  if (typeof IRISH_DEITIES !== "undefined") {
    IRISH_DEITIES.forEach((d) => {
      bucket.deity[d.id] = d;
    });
  }
  if (typeof IRISH_HEROES !== "undefined") {
    IRISH_HEROES.forEach((h) => {
      bucket.hero[h.id] = h;
    });
  }
  if (typeof IRISH_MYTHS !== "undefined") {
    IRISH_MYTHS.forEach((m) => {
      bucket.myth[m.id] = m;
    });
  }
...
window["initPage_celtica-irlandese"] = function () {
  ...
  showIrishSec(
    "cosmo",
    document.querySelector("#page-celtica-irlandese .snav-btn"),
  );
};
...
function _buildIrishPantheon() {
  ...
  grid.innerHTML = IRISH_DEITIES.map(
    (d) => `
...
function showIrishSec(id, btn) {
  document
    .querySelectorAll("#page-celtica-irlandese .sec-block")
    .forEach((s) => s.classList.remove("active"));
  document
    .querySelectorAll("#page-celtica-irlandese .snav-btn")
