...
(function registerSlavicEntities() {
  ...
  if (typeof SLAVIC_DEITIES !== "undefined") {
    SLAVIC_DEITIES.forEach((d) => {
      bucket.deity[d.id] = d;
    });
  }
  if (typeof SLAVIC_HEROES !== "undefined") {
    SLAVIC_HEROES.forEach((h) => {
      bucket.hero[h.id] = h;
    });
  }
  if (typeof SLAVIC_MYTHS !== "undefined") {
    SLAVIC_MYTHS.forEach((m) => {
      bucket.myth[m.id] = m;
    });
  }
...
function _buildSlavicPantheon() {
  ...
  grid.innerHTML = SLAVIC_DEITIES.map(
    (d) => `
    <div class="deity-card" onclick="openSlavicEntity('deity','${d.id}')">
      ...
      <div class="deity-desc">${(() => {
        const t = typeof d.bio === "object" ? d.bio.main || "" : d.bio || "";
        const s = t.slice(0, 160).replace(/\n/g, " ");
        return s
          ? s + (t.length > 160 ? "…" : "")
...
function showSlavicSec(id, btn) {
  document
    .querySelectorAll("#page-slava .sec-block")
    .forEach((s) => s.classList.remove("active"));
  document
    .querySelectorAll("#page-slava .snav-btn")
