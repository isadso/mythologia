...
(function registerWelshEntities() {
  ...
  if (typeof WELSH_DEITIES !== 'undefined') {
    WELSH_DEITIES.forEach(d => { bucket.deity[d.id] = d; });
  }
  if (typeof WELSH_HEROES !== 'undefined') {
    WELSH_HEROES.forEach(h => { bucket.hero[h.id] = h; });
  }
  if (typeof WELSH_MYTHS !== 'undefined') {
    WELSH_MYTHS.forEach(m => { bucket.myth[m.id] = m; });
  }
...
window['initPage_celtica-gallese'] = function() {
  ...
  _buildWelshHeroes();
  _buildWelshMyths();
  showWelshSec('cosmo', document.querySelector('#page-celtica-gallese .snav-btn'));
};
...
function _buildWelshPantheon() {
  ...
  grid.innerHTML = WELSH_DEITIES.map(d => `
...
function _buildWelshHeroes() {
  ...
  c.innerHTML = WELSH_HEROES.map(h => `
...
function showWelshSec(id, btn) {
  document.querySelectorAll('#page-celtica-gallese .sec-block')
    .forEach(s => s.classList.remove('active'));
  document.querySelectorAll('#page-celtica-gallese .snav-btn')
